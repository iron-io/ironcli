...
#   'curl -sSL https://github.com/iron-io/ironcli/releases/download/0.1.6/install_onprem | sh'
# or:
#   'wget -qO-  https://github.com/iron-io/ironcli/releases/download/0.1.6/install_onprem | sh'
#
...
case "$(uname)" in
  Linux)
    $sh_c "$curl /usr/local/bin/iron $url/$release/ironcli_onprem_linux"
    $sh_c "chmod +x /usr/local/bin/iron"
    /usr/local/bin/iron --version
    exit 0
    ;;
  Darwin)
    $sh_c "$curl /usr/local/bin/iron $url/$release/ironcli_onprem_mac"
    $sh_c "chmod +x /usr/local/bin/iron"
    /usr/local/bin/iron --version
    exit 0
    ;;
  WindowsNT)
    $sh_c "$curl $url/$release/ironcli_onprem.exe"
    # TODO how to make executable? chmod?
    ironcli_onprem.exe --version
    exit 0
    ;;
esac
